...
const CATEGORY_CONFIG: CategoryConfig[] = [
  { color: '#7c3aed', geometry: 'torusKnot'   }, // 0 – D2C Brands          – Purple
  { color: '#2563eb', geometry: 'icosahedron'  }, // 1 – Consumer Startups   – Blue
  { color: '#059669', geometry: 'octahedron'   }, // 2 – MSMEs               – Emerald
  { color: '#db2777', geometry: 'sphere'       }, // 3 – SaaS Startups       – Pink
  { color: '#ea580c', geometry: 'box'          }, // 4 – Manufacturing       – Orange
  { color: '#d97706', geometry: 'torus'        }, // 5 – Bharat-focused      – Gold
];
...
function Scene({ activeCategory }: SceneProps) {
  ...
  // GSAP geometry/material swap transition
  useEffect(() => {
    ...
    gsap.to(mesh.scale, {
      ...
      ease: 'power2.in',
      onComplete: () => {
        // Swap geometry + material colour
        setCurrentCategory(activeCategory);

        // Phase 2: spring back up
        gsap.to(mesh.scale, {
          x: 1,
          y: 1,
          z: 1,
